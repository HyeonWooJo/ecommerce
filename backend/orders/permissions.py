...
class OrderDetailIsOwnerOrReadOnly(BasePermission):
    """
    작성자 외 읽기 권한만 부여
...
class OrderIsOwnerOrReadOnly(BasePermission):
    """
    작성자 외 읽기 권한만 부여
    JWT 토큰을 decode, obj의 user와 해당 user가 일치하는지 확인
    ERR03
...
class OwnerAndStaffOrReadOnly_1(BasePermission):
    """
    작성자와 운영자 외 읽기 권한만 부여
    JWT 토큰을 decode, obj의 user와 해당 user가 일치하는지 확인
    ERR03
    """
    message = "[Access Denied: ERR03] 게시글 수정, 삭제 권한이 없습니다."
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if User.objects.get(id=token_user).is_staff:
            return True
        elif obj.order.user.id == token_user:
            return True


class OwnerAndStaffOrReadOnly_2(BasePermission):
    """
    작성자와 운영자 외 읽기 권한만 부여
    JWT 토큰을 decode, obj의 user와 해당 user가 일치하는지 확인
    ERR03
